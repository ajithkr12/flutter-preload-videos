...
import 'package:flutter_preload_videos/isolate.dart';
...
class PreloadBloc extends Bloc<PreloadEvent, PreloadState> {
  ...
  void _mapEventToState(PreloadEvent event, Emitter<PreloadState> emit) async {
    await event.map(
      ...
      onVideoIndexChanged: (e) {
        ...
        emit(state.copyWith(focusedIndex: e.index,isPlaying: true));
      },
      updateUrls: (e) {
  ...
  Future _initializeControllerAtIndex(int index) async {
    if (state.urls.length > index && index >= 0) {
      ...
      final VideoPlayerController _controller = VideoPlayerController.networkUrl(Uri.parse(state.urls[index]));
